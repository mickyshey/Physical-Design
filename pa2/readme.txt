...
9. 執行結果說明：
	主程式： 主程式在執行的時候會先random construct 100 個 BTree ，並計算average area/wire length。
			 接著進入 SA ，過程中會顯示目前的 runtime 、溫度及 current best cost
			 每做完一次 SA 就會輸出已經執行的 SA 次數。
			 若在一次 SA 中有找到 fit 且較好的 cost，則會輸出 find better solution 和此 solution 的 area 和 wire length。
